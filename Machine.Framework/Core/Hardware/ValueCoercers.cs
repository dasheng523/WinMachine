...
public sealed class DefaultValueCoercer : IValueCoercer
{
    ...
    private static Fin<double> CoerceDouble(object? raw)
    {
        ...
        if (raw is string str)
        {
            ...
            return FinFail<double>(Error.New($"无法把字符串转换�?double: {str}"));
        }

        return FinFail<double>(Error.New($"无法�?{raw.GetType().Name} 转换�?double"));
    }
    ...
    private static Fin<Level> CoerceLevel(object? raw)
    {
        ...
        if (raw is string str)
        {
            ...
            return FinFail<Level>(Error.New($"无法把字符串转换�?Level: {str}"));
        }

        return FinFail<Level>(Error.New($"无法�?{raw.GetType().Name} 转换�?Level"));
    }
