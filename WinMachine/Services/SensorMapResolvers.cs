...
public sealed class SensorMapLevelResolver : IResolver<ISensor<Level>>
{
    ...
    private Fin<ISensor<Level>> ResolveByOptions(string logicalName, SensorOptions s)
    {
        return s.Kind switch
        {
            ...
            _ => FinFail<ISensor<Level>>(Error.New($"Sensor={logicalName} Kind={s.Kind} 不能解析�?Level 传感�?))
        };
    }
...
public sealed class SensorMapDoubleResolver : IResolver<ISensor<double>>
{
    ...
    public Fin<ISensor<double>> Resolve(string logicalName)
    {
        ...
        if (opt.IsNone)
        {
            return FinFail<ISensor<double>>(Error.New($"未找�?DoubleSensor 映射: {logicalName}"));
        }
        ...
        return opt.Match(
            Some: s => s.Kind switch
            {
                ...
                _ => FinFail<ISensor<double>>(Error.New($"Sensor={logicalName} Kind={s.Kind} 不能解析�?double 传感�?))
            },
...
public sealed class SensorMapStringResolver : IResolver<ISensor<string>>
{
    ...
    public Fin<ISensor<string>> Resolve(string logicalName)
    {
        ...
        if (opt.IsNone)
        {
            return FinFail<ISensor<string>>(Error.New($"未找�?StringSensor 映射: {logicalName}"));
        }
        ...
        return opt.Match(
            Some: s => s.Kind switch
            {
                ...
                _ => FinFail<ISensor<string>>(Error.New($"Sensor={logicalName} Kind={s.Kind} 不能解析�?string 传感�?))
            },
