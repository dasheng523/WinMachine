...
public sealed class CylinderResolver : ICylinderResolver
{
    ...
    public Fin<ISingleSolenoidCylinder> Resolve(string name)
    {
        ...
        {
            return FinFail<ISingleSolenoidCylinder>(Error.New("未配置 System.CylinderMap"));
        }
        ...
        if (opt is null)
        {
            return FinFail<ISingleSolenoidCylinder>(Error.New($"未找到气缸映射: {name}"));
        }

        if (string.IsNullOrWhiteSpace(opt.ValveDo))
        {
            return FinFail<ISingleSolenoidCylinder>(Error.New($"气缸 {name} 未配置 ValveDo"));
        }
