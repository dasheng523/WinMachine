    ...
    public Fin<double> Read()
    {
        try
        {
            return _pool.GetOrCreate(_conn).Bind(master =>
            {
                ...
                {
                    return FinFail<double>(Error.New($"ModbusHoldingRegister 读取长度不足: {Name}"));
                }

                // 兼容旧代码：Count=2 时 regs[0]=low, regs[1]=high，组合成有符号 Int32
                double raw = _count switch
                {
                    1 => regs[0],
                    2 => ToInt32LowHigh(regs[0], regs[1]),
                    _ => throw new NotSupportedException($"仅支持 Count=1/2, 当前 Count={_count}")
                };
        ...
        catch (Exception ex)
        {
            return FinFail<double>(Error.New(new Exception(
                $"ModbusHoldingRegister 读取失败: {Name} slave={_slaveId} addr={_address} count={_count}, {ex.Message}", ex)));
        }
    }
