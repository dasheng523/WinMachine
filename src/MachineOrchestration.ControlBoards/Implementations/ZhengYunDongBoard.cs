...
/// <summary>
/// 正运动控制板实现
/// 集成正运动 SDK，提供统一的控制板接口
/// 包含错误处理和重试逻辑
/// </summary>
public sealed class ZhengYunDongBoard : IControlBoard, IDisposable
{
    private readonly IZhengYunDongSdk _sdk;
    ...
    public ZhengYunDongBoard(IZhengYunDongSdk sdk, ZhengYunDongBoardConfig config)
    {
    ...
    public async Task<Either<ControlBoardError, Unit>> Initialize()
    {
            ...
            if (!connected)
            {
                return Left<ControlBoardError, Unit>(
                    new ControlBoardError.ConnectionError(
                        $"Failed to connect to ZhengYunDong board at {ipAddress}:{port}"));
            }
        ...
        catch (Exception ex)
        {
            return Left<ControlBoardError, Unit>(
                new ControlBoardError.ConnectionError(
                    "Failed to initialize ZhengYunDong board", ex));
        }
    }
...
/// 正运动控制板配置
/// </summary>
public sealed record ZhengYunDongBoardConfig(
    string IpAddress,
    int Port,
    int MaxRetries,
